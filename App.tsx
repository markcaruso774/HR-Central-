  ...
  return (
    <div>
      ...
      <p style={{ color: COLORS.muted, marginBottom: 28, fontSize: 14 }}>
        Welcome to your HR Command Centre
      </p>
      <div
        style={{
          display: 'grid',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <Card>
          <h3 style={{ color: COLORS.text, marginBottom: 16, fontSize: 15 }}>
            Recent Employees
          </h3>
          {employees
            .slice(-5)
    ...
    <div>
      ...
      >
        ...
        <Card>
          ...
          >
            <h3 style={{ color: COLORS.text, fontSize: 15 }}>
                ...
                {filtered.map((e) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '10px' }}>
                      <Badge color={COLORS.accent}>{e.department || '—'}</Badge>
                    </td>
                    <td
                      style={{
                        padding: '10px',
...
function Attendance({ employees, attendance, setAttendance }) {
  ...
  const getStatus = (empId) =>
    attendance.find((a) => a.employeeId === empId && a.date === date)?.status ||
    null;
    ...
    <div>
      ...
      <Card>
        ...
        >
          <thead>
            <tr style={{ borderBottom: `2px solid ${COLORS.border}` }}>
              {['Employee', 'Department', 'Status', 'Mark Attendance'].map(
                (h) => (
                  <th
                    key={h}
                    style={{
                  ...
                  <td style={{ padding: '12px' }}>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <Btn
                        small
                        color={COLORS.accent3}
                        onClick={() => mark(e.id, e.name, 'Present')}
                      >
                        Present
                      </Btn>
                      <Btn
                        small
                        color={COLORS.danger}
                        onClick={() => mark(e.id, e.name, 'Absent')}
                      >
                        Absent
                      </Btn>
                      <Btn
        ...
        {employees.length === 0 && (
          <p style={{ color: COLORS.muted, textAlign: 'center', padding: 30 }}>
            Add employees first.
          </p>
        )}
      </Card>
      ...
      >
        ...
        <Card>
          ...
          >
            <thead>
              <tr style={{ borderBottom: `2px solid ${COLORS.border}` }}>
                {['Employee', 'Type', 'Period', 'Status', 'Actions'].map(
                  (h) => (
                    <th
                      key={h}
                      style={{
            ...
            <tbody>
              {leaveRequests.map((l) => (
                ...
                >
                  ...
                  <td style={{ padding: '10px' }}>
                    <Badge color={COLORS.accent2}>{l.type}</Badge>
                  </td>
                  <td
                    style={{
                      padding: '10px',
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <input
                        ...
                        onChange={(ev) =>
                          setOv(e.id, 'salary', ev.target.value)
                        }
                        style={{
                          width: 110,
                          background: '#0f1117',
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <input
                        ...
                        placeholder="0"
                        onChange={(ev) =>
                          setOv(e.id, 'allowance', ev.target.value)
                        }
                        style={{
                          width: 90,
                          background: '#0f1117',
...
function Contracts() {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (rows.length > 0) {
          ...
          setStatus(
            `✓ Loaded ${dataRows.length} staff from "${wb.SheetNames[0]}". Now select the Contract Template sheet and set up your mappings below.`
          );
        }
      } catch (err) {
  ...
  const generate = () => {
    if (!workbook || !contractSheet || staffData.length === 0) {
      setStatus(
        '⚠ Please upload file, select contract sheet, and add at least one mapping.'
      );
      return;
    }
    const results = [];
    staffData.forEach((staff, idx) => {
      try {
        const wsClone = JSON.parse(
          JSON.stringify(workbook.Sheets[contractSheet])
        );
        mappings.forEach(({ column, cell }) => {
          ...
          if (wsClone[cellKey]) {
            wsClone[cellKey].v = value;
            wsClone[cellKey].w = String(value);
            wsClone[cellKey].t = 's';
          } else {
            wsClone[cellKey] = { v: value, w: String(value), t: 's' };
          }
        });
        const newWb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(newWb, wsClone, 'Contract');
        const xlsxData = XLSX.write(newWb, {
          bookType: 'xlsx',
          type: 'base64',
        });
        results.push({
          name: staff[columns[0]] || `Staff_${idx + 1}`,
          data: xlsxData,
        });
      } catch (err) {
        console.error(err);
    ...
    setStatus(
      `✓ Generated ${results.length} contracts successfully! Click each name to download.`
    );
  };
  ...
  const downloadAll = () =>
    generated.forEach((item, i) =>
      setTimeout(() => downloadOne(item), i * 400)
    );
  ...
  return (
    <div>
      ...
      >
        ...
        <strong style={{ color: COLORS.accent }}>Sheet 1</strong> = Staff Data
        (column headers become your fields) and{' '}
        <strong style={{ color: COLORS.accent2 }}>another sheet</strong> = your
        contract template. Map each column to the exact cell in the template and
        generate all contracts at once.
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <Card>
          ...
          >
            ...
            <div style={{ color: COLORS.muted, fontSize: 13 }}>
              {file ? `✓ ${file.name}` : 'Click to upload your .xlsx file'}
            </div>
          </div>
          <input
            ref={fileRef}
          ...
          {sheetOptions.length > 0 && (
            <>
              ...
              >
                Step 2 — Select Contract Template Sheet
              </h3>
              <SelectField
                label="Contract Sheet"
                value={contractSheet}
                onChange={setContractSheet}
              ...
              >
                Detected:{' '}
                <strong style={{ color: COLORS.accent3 }}>
        ...
        <Card>
          ...
          >
            E.g. "Staff Name" (from Sheet 1) → G5 (cell in contract template)
          </p>
          {mappings.map((m, i) => (
            <div
              key={i}
            ...
            >
              <div style={{ flex: 1 }}>
                <SelectField
                  label={i === 0 ? 'Column (Sheet 1)' : ''}
                  value={m.column}
                  onChange={(v) => updateMapping(i, 'column', v)}
                  options={columns.map((c) => ({ value: c, label: c }))}
                />
              </div>
              <div
                style={{ color: COLORS.muted, paddingBottom: 14, fontSize: 20 }}
              >
                →
              </div>
              <div style={{ flex: 1 }}>
                <Input
                  label={i === 0 ? 'Target Cell' : ''}
                  value={m.cell}
                  onChange={(v) => updateMapping(i, 'cell', v)}
                  placeholder="e.g. G5"
                />
              </div>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
            <Btn small color={COLORS.muted} onClick={addMapping}>
              + Add Mapping
            </Btn>
          </div>
          <div style={{ marginTop: 20 }}>
            <Btn
              onClick={generate}
              disabled={!workbook || mappings.length === 0}
            >
      ...
      {status && (
        <div
          style={{
            ...
            border: `1px solid ${COLORS.border}`,
            borderRadius: 8,
            color: status.startsWith('✓') ? COLORS.accent3 : COLORS.accent2,
            fontSize: 13,
          }}
        >
          {status}
      ...
      {generated.length > 0 && (
        <Card style={{ marginTop: 20 }}>
          ...
          >
            <h3 style={{ color: COLORS.text, fontSize: 15 }}>
              Generated Contracts ({generated.length})
            </h3>
            <Btn small color={COLORS.accent} onClick={downloadAll}>
              ⬇ Download All
            </Btn>
          </div>
          <div
...
function Payslips() {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (rows.length > 0) {
          ...
          setStatus(
            `✓ Loaded ${dataRows.length} staff. Now select payslip template sheet and map your columns.`
          );
        }
      } catch (err) {
  ...
  const generate = () => {
    if (!workbook || !payslipSheet || staffData.length === 0) {
      setStatus(
        '⚠ Please upload file, select payslip sheet, and add mappings.'
      );
      return;
    }
    const results = [];
    staffData.forEach((staff, idx) => {
      try {
        const wsClone = JSON.parse(
          JSON.stringify(workbook.Sheets[payslipSheet])
        );
        mappings.forEach(({ column, cell }) => {
          if (!column || !cell) return;
          const value = staff[column] ?? '';
    ...
    setGenerated(results);
    setStatus(
      `✓ Generated ${results.length} payslips! Click each name or use Download All.`
    );
  };
  ...
  return (
    <div>
      ...
      >
        Upload your Excel file —{' '}
        <strong style={{ color: COLORS.accent }}>Sheet 1</strong> = Staff salary
        data and{' '}
        <strong style={{ color: COLORS.accent2 }}>another sheet</strong> = your
        payslip template. Map each column to the correct cell and generate all
        payslips instantly.
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <Card>
          ...
          {sheetOptions.length > 0 && (
            <>
              ...
              >
                Step 2 — Select Payslip Template Sheet
              </h3>
              <SelectField
                label="Payslip Sheet"
                value={payslipSheet}
                onChange={setPayslipSheet}
                options={sheetOptions}
              />
              <div
                style={{
        ...
        <Card>
          ...
          >
            E.g. "Basic Salary" → F8, "Staff Name" → C3, "Net Pay" → F15
          </p>
          {mappings.map((m, i) => (
            <div
              key={i}
      ...
      {generated.length > 0 && (
        <Card style={{ marginTop: 20 }}>
          ...
          >
            <h3 style={{ color: COLORS.text, fontSize: 15 }}>
              Generated Payslips ({generated.length})
            </h3>
            <Btn small color={COLORS.accent} onClick={downloadAll}>
              ⬇ Download All
            </Btn>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div style={{ fontSize: 18, color: COLORS.accent, fontWeight: 800 }}>
            HR Central
          </div>
          <div
            style={{
          ...
          {modules.map((m) => (
            <div
              ...
              style={{
                ...
                marginBottom: 3,
                background:
                  active === m.id ? COLORS.accent + '18' : 'transparent',
                borderLeft:
                  active === m.id
                    ? `3px solid ${COLORS.accent}`
                    : '3px solid transparent',
                color: active === m.id ? COLORS.accent : COLORS.muted,
                fontSize: 14,
