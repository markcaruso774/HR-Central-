...
function EmployeeProfile({ employee, onClose }) {
  ...
  // Build last 30 days for line chart
  const last30 = [];
  for (let i = 29; i >= 0; i--) {
  ...
  const chartW = 520; const chartH = 120;
  const points = last30.map((d, i) => `${(i / 29) * chartW},${chartH - (d.value / 3) * chartH}`).join(" ");

  return (
    <div style={{ position: "fixed", top: 0, right: 0, width: 580, height: "100vh", background: C.bgDeep, borderLeft: `1px solid ${C.border}`, zIndex: 1000, overflowY: "auto", boxShadow: "-8px 0 40px rgba(0,0,0,0.5)", animation: "slideIn 0.3s ease" }}>
      <style>{`@keyframes slideIn { from { transform: translateX(100%); } to { transform: translateX(0); } }`}</style>

      {/* Header */}
      <div style={{ padding: "24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: C.bgDeep, zIndex: 10 }}>
        <div>
          <h2 style={{ color: C.white, fontSize: 20, fontWeight: 800, margin: 0 }}>{employee.name}</h2>
          <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
            <Badge color={C.accent}>{employee.designation || "Staff"}</Badge>
            <Badge color={C.muted}>{employee.location || "—"}</Badge>
          </div>
        </div>
        <button onClick={onClose} style={{ background: C.border, border: "none", borderRadius: 8, padding: "8px 14px", color: C.text, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>Close ✕</button>
      </div>

      <div style={{ padding: 24 }}>
        {/* Personal Info */}
        <Card style={{ marginBottom: 16 }}>
          <h3 style={{ color: C.accent, fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 16 }}>Personal Information</h3>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              ["Education", employee.education_qualifications],
              ["Certification", employee.professional_certification],
            ].map(([label, value]) => (
        ...
        <Card style={{ marginBottom: 16 }}>
          ...
          {/* Line Chart */}
          {loading ? <div style={{ color: C.muted, fontSize: 13 }}>Loading chart...</div> : (
            <div>
              ...
              <div style={{ overflowX: "auto" }}>
                <svg width={chartW} height={chartH + 30} style={{ display: "block" }}>
                  {/* Grid lines */}
                  {[0,1,2,3].map(v => (
                    <line key={v} x1="0" y1={chartH - (v/3)*chartH} x2={chartW} y2={chartH - (v/3)*chartH} stroke={C.border} strokeWidth="1" strokeDasharray="4,4" />
                  ))}
                  {/* Labels */}
                  {["Absent","Late","Present"].map((l,i) => (
                    <text key={l} x="4" y={chartH - ((i+1)/3)*chartH + 4} fill={i===0?C.danger:i===1?C.warning:C.success} fontSize="9" fontWeight="700">{l}</text>
                  ))}
                  {/* Area fill */}
                  <defs>
                  ...
                  {/* Dots */}
                  {last30.map((d, i) => d.value > 0 && (
                    <circle key={i} cx={(i/29)*chartW} cy={chartH-(d.value/3)*chartH} r="3"
                      fill={d.status==="Present"?C.success:d.status==="Late"?C.warning:C.danger} />
                  ))}
                  {/* X axis labels - show every 5th */}
                  {last30.filter((_,i) => i % 5 === 0).map((d, i) => (
                    <text key={i} x={(i*5/29)*chartW} y={chartH+20} fill={C.muted} fontSize="8" textAnchor="middle">{d.label}</text>
                  ))}
                </svg>
              </div>
            </div>
