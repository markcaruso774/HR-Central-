  ...
  const handle = async () => {
    ...
    try {
      if (mode === "signup") {
        ...
        onLogin({ ...data.user, role: isHR ? "hr" : "employee" });
      }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, position: "relative" }}>
        ...
        <p style={{ textAlign: "center", color: C.muted, fontSize: 11, marginTop: 16 }}>HR tip: Use an email with "hr" or "admin" to get full HR access</p>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        ...
        const role = email.toLowerCase().includes("hr") || email.toLowerCase().includes("admin") ? "hr" : "employee";
  ...
  const visibleModules = isHR ? modules : modules.filter(m => ["dashboard", "employees", "attendance", "leave"].includes(m.id));
      ...
      <div style={{ width: 220, background: C.bgDeep, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", flexShrink: 0, position: "fixed", top: 0, left: 0, height: "100vh", zIndex: 100 }}>
        ...
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}` }}>
          <div style={{ background: C.accent + "15", borderRadius: 10, padding: "10px 12px" }}>
            <div style={{ fontSize: 12, color: C.text, fontWeight: 600, marginBottom: 2 }}>{user.email?.split("@")[0]}</div>
            <Badge color={isHR ? C.accent : C.success} style={{ fontSize: 10 }}>{isHR ? "HR Admin" : "Employee"}</Badge>
          </div>
        </div>
