// HR-CENTRAL-BUILD-v3-OCR-PAYROLL-2026
...
function Employees({ employees, setEmployees, isHR, setSelectedEmployee }) {
  ...
  const downloadTemplate = async () => {
    ...
    window.XLSX.utils.book_append_sheet(wb, window.XLSX.utils.json_to_sheet(template), "Staff Template");
    window.XLSX.writeFile(wb, "Staff_Upload_Template.xlsx");
  };
...
function Attendance({ employees, isHR, setSelectedEmployee }) {
  ...
  const [ocrAbsent, setOcrAbsent] = useState([]);
  const [ocrError, setOcrError] = useState("");
  const ocrRef1 = useRef();
  const ocrRef2 = useRef();
  ...
  const mark = async (emp, status) => {
    const existing = records.find(r => String(r.employee_id) === String(emp.id));
    if (existing && existing.status === status) {
      await supabase.from("attendance").delete().eq("id", existing.id);
      setRecords(records.filter(r => String(r.employee_id) !== String(emp.id)));
      return;
    }
    const payload = { employee_id: emp.id, employee_name: emp.name, date, status,
      late_duration: status === "Late" ? (lateMinutes[emp.id] || "") : status === "Absent" ? (absentReasons[emp.id] || "") : null };
    if (existing) {
      await supabase.from("attendance").update(payload).eq("id", existing.id);
      setRecords(records.map(r => String(r.employee_id) === String(emp.id) ? { ...r, ...payload } : r));
    } else {
  ...
  // Load Tesseract OCR library (v2 - simpler API)
  const loadTesseract = () => new Promise((res, rej) => {
    ...
    s.onload = () => {
      if (window.Tesseract) res();
      else rej(new Error("Tesseract loaded but not available"));
    };
    s.onerror = () => rej(new Error("Failed to load Tesseract. Check internet connection."));
    document.head.appendChild(s);
  });

  // Run OCR on a single image file, returns extracted text
  const runOCR = (file) => new Promise((res, rej) => {
    const reader = new FileReader();
    reader.onload = async (e) => {
      try {
        const { data: { text } } = await window.Tesseract.recognize(e.target.result, "eng", { logger: () => {} });
        res(text);
      } catch(err) { rej(err); }
    };
    reader.onerror = rej;
  ...
  // Parse OCR text into rows of {rawName, timeIn}
  const parseOCRText = text => {
    const lines = text.split("\n").map(l => l.trim()).filter(l => l.length > 2);
    const rows = [];
    lines.forEach(line => {
      if (/^(date|serial|s\/n|name|time in|time out|sign)/i.test(line) && line.length < 40) return;
      const timeMatch = line.match(/(\d{1,2}[:.]\d{2}\s*(?:am|pm)?)/i);
      if (!timeMatch) return;
      const timeIdx = line.indexOf(timeMatch[0]);
      let namePart = line.substring(0, timeIdx).trim().replace(/^\d{1,3}[.\s]+/, "").trim();
      if (namePart.length > 1) rows.push({ rawName: namePart, timeIn: timeMatch[0].trim() });
    });
    return rows;
  };

  // Process uploaded image(s) with Tesseract OCR
  ...
  const bestMatch = (ocrName, empList) => {
    const name = ocrName.toLowerCase().trim();
    let best = null, bestScore = Infinity;
    empList.forEach(emp => {
      const empName = emp.name.toLowerCase();
      const score = levenshtein(name, empName);
      const threshold = Math.max(3, Math.floor(empName.length * 0.4));
      if (score < bestScore && score <= threshold) { best = emp; bestScore = score; }
    });
    return best;
  };

  const processOCR = async () => {
    if (ocrPages.length === 0) { setOcrError("Please upload at least one page."); return; }
    ...
    try {
      ...
      for (const page of ocrPages) {
        const text = await runOCR(page.file);
        rawTexts.push(text);
        const rows = parseOCRText(text);
        allRows = [...allRows, ...rows];
      }
      
      // If no rows extracted, show raw OCR text so HR can see what was read
      if (allRows.length === 0) {
        const preview = rawTexts.join("\n\n--- PAGE 2 ---\n\n").substring(0, 800);
        setOcrError("OCR could not extract names. Raw text read:\n\n" + (preview || "(nothing extracted - image may be unclear)"));
        setOcrProcessing(false);
        return;
      }

      // Deduplicate by rawName
      const seen = new Set();
      allRows = allRows.filter(r => {
        const k = r.rawName.toLowerCase();
      ...
      const { data: leaveData } = await supabase.from("leave_requests")
        .select("*").eq("status", "approved").lte("start_date", date).gte("end_date", date);
      const onLeave = new Set((leaveData || []).map(l => l.employee_id));

      // Match OCR rows to employees
      const matched = [], flagged = [];
      const matchedIds = new Set();
      const CUTOFF = 8 * 60 + 30; // 8:30 AM

      allRows.forEach(row => {
        const emp = bestMatch(row.rawName, employees);
        const timeInMins = parseTime(row.timeIn);
        let status = "Present", lateDuration = "";
        if (timeInMins !== null && timeInMins > CUTOFF) {
          status = "Late";
          const diff = timeInMins - CUTOFF;
          lateDuration = (Math.floor(diff / 60) > 0 ? Math.floor(diff / 60) + "h " : "") + (diff % 60) + "min";
        }
        if (emp) {
          matchedIds.add(emp.id);
          matched.push({ emp, rawName: row.rawName, timeIn: row.timeIn, status, lateDuration, manualEmp: null });
        } else {
          flagged.push({ emp: null, rawName: row.rawName, timeIn: row.timeIn, status, lateDuration, manualEmp: null });
        }
      });

      // Absent: not on register and not on leave
      const absent = employees.filter(e => !matchedIds.has(e.id) && !onLeave.has(e.id));
      setOcrResults([...matched, ...flagged]);
      setOcrAbsent(absent);
      setOcrStep(2);
    } catch(err) {
      setOcrError("Could not read the image: " + err.message + ". Please try a clearer photo.");
    }
    setOcrProcessing(false);
  };

  // Confirm and save all OCR results to database
  const confirmOCR = async () => {
    setOcrProcessing(true);
    const toSave = [];
    for (const r of ocrResults) {
      const emp = r.emp || r.manualEmp;
      if (!emp) continue;
      toSave.push({ employee_id: emp.id, employee_name: emp.name, date, status: r.status,
        late_duration: r.status === "Late" ? r.lateDuration : null });
    }
    for (const emp of ocrAbsent) {
      toSave.push({ employee_id: emp.id, employee_name: emp.name, date, status: "Absent", late_duration: null });
    }
    for (const payload of toSave) {
      const existing = records.find(r => r.employee_id === payload.employee_id);
      if (existing) {
        await supabase.from("attendance").update(payload).eq("id", existing.id);
      } else {
    ...
    await loadRecords();
    setShowOCR(false);
    setOcrStep(1); setOcrPages([]); setOcrResults([]); setOcrAbsent([]);
    setOcrProcessing(false);
  };

  const addPage = (e, pageNum) => {
    const file = e.target.files[0]; if (!file) return;
    const preview = URL.createObjectURL(file);
    setOcrPages(prev => { const u = [...prev]; u[pageNum-1] = { file, preview }; return u; });
  };
  ...
  const unmarked = total - records.filter(r => filteredEmployees.some(e => String(e.id) === String(r.employee_id))).length;
  ...
  return (
    <div>
      ...
      {/* Date selector + Upload Register button */}
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        {isHR && (
          <Btn color={C.accent} onClick={() => { setShowOCR(true); setOcrStep(1); setOcrPages([]); setOcrResults([]); setOcrAbsent([]); setOcrError(""); }}>
            📷 Upload Register
          </Btn>
        )}
      </div>
      ...
      {locations.length > 0 && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
          <button onClick={() => setSelectedLocation("")} style={{ padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: !selectedLocation ? C.accent : C.card, color: !selectedLocation ? "#fff" : C.muted }}>All</button>
          {locations.map(loc => (
            <button key={loc} onClick={() => setSelectedLocation(loc)} style={{ padding: "6px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, background: selectedLocation === loc ? C.accent : C.card, color: selectedLocation === loc ? "#fff" : C.muted }}>{loc}</button>
          ))}
        </div>
      )}
      ...
      <Card style={{ marginBottom: 16, padding: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
          {[
            ...
            { label: "Unmarked", value: unmarked, color: C.muted },
          ].map(s => (
            <div key={s.label} style={{ background: C.bgDeep, borderRadius: 10, padding: 12, textAlign: "center", border: `1px solid ${C.border}` }}>
      ...
      <Card>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${C.border}` }}>
                <th style={{ padding: "10px 12px", textAlign: "left", color: C.muted, fontWeight: 600, fontSize: 11, textTransform: "uppercase" }}>Employee</th>
                <th style={{ padding: "10px 12px", textAlign: "left", color: C.muted, fontWeight: 600, fontSize: 11, textTransform: "uppercase" }}>Location</th>
                <th style={{ padding: "10px 12px", textAlign: "center", color: C.muted, fontWeight: 600, fontSize: 11, textTransform: "uppercase" }}>Status</th>
                <th style={{ padding: "10px 12px", textAlign: "left", color: C.muted, fontWeight: 600, fontSize: 11, textTransform: "uppercase" }}>Details</th>
              </tr>
            </thead>
            <tbody>
              {filteredEmployees.map(emp => {
                const rec = records.find(r => String(r.employee_id) === String(emp.id));
                const status = rec?.status;
                return (
                  <tr key={emp.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                    <td style={{ padding: "10px 12px" }}>
                      <span onClick={() => setSelectedEmployee(emp)} style={{ color: C.accent, cursor: "pointer", fontWeight: 600 }}>{emp.name}</span>
                    </td>
                    <td style={{ padding: "10px 12px", color: C.muted, fontSize: 12 }}>{emp.location || "—"}</td>
                    <td style={{ padding: "10px 12px" }}>
                      <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                        {["Present", "Late", "Absent"].map(s => (
                          <button key={s} onClick={() => mark(emp, s)} style={{
                            padding: "4px 10px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700,
                            background: status === s ? (s === "Present" ? C.success : s === "Late" ? C.warning : C.danger) : C.bgDeep,
                            color: status === s ? "#fff" : C.muted, transition: "all 0.15s"
                          }}>{s}</button>
                        ))}
                      </div>
                    </td>
                    <td style={{ padding: "10px 12px" }}>
                      {status === "Late" && (
                        <input placeholder="e.g. 45min" value={lateMinutes[emp.id] || ""}
                          onChange={e => setLateMinutes(prev => ({ ...prev, [emp.id]: e.target.value }))}
                          onBlur={() => { if (rec) { supabase.from("attendance").update({ late_duration: lateMinutes[emp.id] || "" }).eq("id", rec.id); } }}
                          style={{ background: C.bgDeep, border: `1px solid ${C.border}`, borderRadius: 7, padding: "4px 8px", color: C.text, fontSize: 12, width: 90, outline: "none" }} />
                      )}
                      {status === "Absent" && (
                        <input placeholder="Reason" value={absentReasons[emp.id] || ""}
                          onChange={e => setAbsentReasons(prev => ({ ...prev, [emp.id]: e.target.value }))}
                          onBlur={() => { if (rec) { supabase.from("attendance").update({ late_duration: absentReasons[emp.id] || "" }).eq("id", rec.id); } }}
                          style={{ background: C.bgDeep, border: `1px solid ${C.border}`, borderRadius: 7, padding: "4px 8px", color: C.text, fontSize: 12, width: 120, outline: "none" }} />
                      )}
                    </td>
                  </tr>
                );
      ...
      {/* OCR UPLOAD MODAL */}
      {showOCR && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 1200, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}>
          <div style={{ background: C.card, borderRadius: 20, width: "min(680px,100%)", border: `1px solid ${C.border}`, boxShadow: "0 24px 80px rgba(0,0,0,0.7)", maxHeight: "92vh", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ padding: "18px 22px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
              <div>
                <h2 style={{ color: C.white, fontSize: 16, fontWeight: 800, margin: 0 }}>📷 Upload Attendance Register</h2>
                <p style={{ color: C.muted, fontSize: 12, marginTop: 3 }}>
                  {date === today ? "Today" : new Date(date).toLocaleDateString("en", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
                </p>
              </div>
              <button onClick={() => setShowOCR(false)} style={{ background: "transparent", border: "none", color: C.muted, fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "20px 22px" }}>
              ...
              {ocrStep === 1 && (
                <div>
                  <p style={{ color: C.muted, fontSize: 13, marginBottom: 16, lineHeight: 1.6 }}>
                    Take a clear photo of your attendance register. If it spans 2 pages, upload both below.
                  </p>

                  {/* Page 1 */}
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: C.success, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>Page 1 *</div>
                    <div onClick={() => ocrRef1.current.click()} style={{ border: `2px dashed ${ocrPages[0] ? C.success : C.border}`, borderRadius: 12, padding: 20, textAlign: "center", cursor: "pointer", background: ocrPages[0] ? C.success+"08" : "transparent" }}>
                      {ocrPages[0] ? (
                        <div>
                          <img src={ocrPages[0].preview} alt="Page 1" style={{ maxWidth: "100%", maxHeight: 200, borderRadius: 8, objectFit: "contain" }} />
                          <div style={{ color: C.success, fontSize: 12, fontWeight: 600, marginTop: 8 }}>✓ Uploaded — click to change</div>
                        </div>
                      ) : (
                        <div>
                          <div style={{ fontSize: 32, marginBottom: 8 }}>📷</div>
                          <div style={{ color: C.muted, fontSize: 13, fontWeight: 600 }}>Click to upload / take photo of Page 1</div>
                        </div>
                      )}
                    </div>
                    <input ref={ocrRef1} type="file" accept="image/*" onChange={e => addPage(e, 1)} style={{ display: "none" }} />
                  </div>

                  {/* Page 2 */}
                  <div style={{ marginBottom: 16 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>Page 2 (optional)</div>
                    <div onClick={() => ocrRef2.current.click()} style={{ border: `2px dashed ${ocrPages[1] ? C.success : C.border}`, borderRadius: 12, padding: 20, textAlign: "center", cursor: "pointer", background: ocrPages[1] ? C.success+"08" : "transparent" }}>
                      {ocrPages[1] ? (
                        <div>
                          <img src={ocrPages[1].preview} alt="Page 2" style={{ maxWidth: "100%", maxHeight: 200, borderRadius: 8, objectFit: "contain" }} />
                          <div style={{ color: C.success, fontSize: 12, fontWeight: 600, marginTop: 8 }}>✓ Uploaded — click to change</div>
                        </div>
                      ) : (
                        <div>
                          <div style={{ fontSize: 32, marginBottom: 8 }}>📷</div>
                          <div style={{ color: C.muted, fontSize: 13, fontWeight: 600 }}>Click to upload / take photo of Page 2</div>
                        </div>
                      )}
                    </div>
                    <input ref={ocrRef2} type="file" accept="image/*" onChange={e => addPage(e, 2)} style={{ display: "none" }} />
                  </div>

                  {ocrError && (
                    <div style={{ background: C.danger+"18", border: `1px solid ${C.danger}44`, borderRadius: 9, padding: "10px 14px", color: C.danger, fontSize: 12, marginBottom: 12 }}>
                      {ocrError}
                    </div>
                  )}

                  <div style={{ display: "flex", gap: 10, marginTop: 8, flexWrap: "wrap" }}>
                    <Btn color={C.success} onClick={processOCR}>
                      {ocrProcessing ? "⏳ Reading register..." : "⚡ Read Register"}
                    </Btn>
                    <Btn outline color={C.muted} onClick={() => setShowOCR(false)}>Cancel</Btn>
                  </div>
                  {ocrProcessing && (
                    <div style={{ marginTop: 14, color: C.muted, fontSize: 12, lineHeight: 1.8 }}>
                      🔍 Analysing image with OCR... This may take 20–40 seconds. Please wait.
                    </div>
                  )}
                </div>
              )}
              ...
              {ocrStep === 2 && (
                <div>
                  <p style={{ color: C.muted, fontSize: 13, marginBottom: 16, lineHeight: 1.6 }}>
                    Review the results below. You can change any status or resolve flagged names before saving.
                  </p>

                  {/* Matched */}
                  {ocrResults.filter(r => r.emp).length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: C.success, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>
                        ✅ Matched Staff ({ocrResults.filter(r => r.emp).length})
                      </div>
                      {ocrResults.map((r, i) => !r.emp ? null : (
                        <div key={i} style={{ background: C.bgDeep, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", marginBottom: 6, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontWeight: 700, color: C.white, fontSize: 13 }}>{r.emp.name}</div>
                            <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>Read as: "{r.rawName}" · Time In: {r.timeIn || "—"}</div>
                          </div>
                          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                            {["Present","Late","Absent"].map(s => (
                              <button key={s} onClick={() => setOcrResults(prev => prev.map((x, j) => j === i ? {...x, status: s} : x))}
                                style={{ padding: "3px 10px", borderRadius: 7, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700,
                                  background: r.status === s ? (s==="Present"?C.success:s==="Late"?C.warning:C.danger) : C.card,
                                  color: r.status === s ? "#fff" : C.muted }}>{s}</button>
                            ))}
                            {r.status === "Late" && (
                              <input value={r.lateDuration} onChange={e => setOcrResults(prev => prev.map((x, j) => j === i ? {...x, lateDuration: e.target.value} : x))}
                                placeholder="e.g. 45min" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 7, padding: "3px 8px", color: C.text, fontSize: 11, width: 80, outline: "none" }} />
                            )}
                          </div>
                        </div>
                      ))}
                  ...
                  {ocrResults.filter(r => !r.emp).length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: C.warning, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>
                        ⚠️ Unrecognised Names ({ocrResults.filter(r => !r.emp).length}) — Please resolve
                      </div>
                      {ocrResults.map((r, i) => r.emp ? null : (
                        <div key={i} style={{ background: C.warning+"10", border: `1px solid ${C.warning}44`, borderRadius: 10, padding: "10px 14px", marginBottom: 6 }}>
                          <div style={{ fontWeight: 700, color: C.warning, fontSize: 13, marginBottom: 8 }}>"{r.rawName}" · Time In: {r.timeIn || "—"}</div>
                          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                            <select value={r.manualEmp?.id || ""} onChange={e => {
                              const emp = employees.find(x => x.id === e.target.value);
                              setOcrResults(prev => prev.map((x, j) => j === i ? {...x, manualEmp: emp || null} : x));
                            }} style={{ background: C.bgDeep, border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 10px", color: C.text, fontSize: 12, outline: "none", flex: 1 }}>
                          ...
                          {r.manualEmp && <div style={{ fontSize: 11, color: C.success, marginTop: 6 }}>✓ Will save as: {r.manualEmp.name}</div>}
                          {!r.manualEmp && <div style={{ fontSize: 11, color: C.muted, marginTop: 6 }}>⚠ Not selected — will be skipped</div>}
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {ocrAbsent.length > 0 && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: C.danger, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 }}>
                        ❌ Absent Staff ({ocrAbsent.length}) — Not on register & not on approved leave
                      </div>
                      <div style={{ background: C.danger+"10", border: `1px solid ${C.danger}33`, borderRadius: 10, padding: "12px 14px" }}>
                        {ocrAbsent.map((e, i) => (
                          <div key={e.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "5px 0", borderBottom: i < ocrAbsent.length-1 ? `1px solid ${C.border}` : "none" }}>
                            <span style={{ color: C.text, fontSize: 13 }}>{e.name}</span>
                            <button onClick={() => setOcrAbsent(prev => prev.filter(x => x.id !== e.id))}
                              style={{ background: "transparent", border: "none", color: C.muted, cursor: "pointer", fontSize: 12 }}>Remove ✕</button>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
                    <Btn color={C.success} onClick={confirmOCR}>
                      {ocrProcessing ? "⏳ Saving..." : "✅ Confirm & Save Attendance"}
                    </Btn>
                    <Btn outline color={C.muted} onClick={() => setOcrStep(1)}>← Re-upload</Btn>
                    <Btn outline color={C.muted} onClick={() => setShowOCR(false)}>Cancel</Btn>
                  </div>
                </div>
              )}
...
function Payroll({ employees }) {
  // ── STATE ──
  const [tab, setTab] = useState('upload');        // 'upload' | 'manual' | 'results'
  ...
  const [colMap, setColMap] = useState({ name:-1, pos:-1, cum:-1, dur:-1, other:-1, rent:-1, ded:-1 });
  const [showColMap, setShowColMap] = useState(false);

  // Manual tab state
  const [manualStaff, setManualStaff] = useState([]);
  const [form, setForm] = useState({ name:'', position:'', cumulative:'', duration:'6', otherIncome:'0', rent:'0', otherDeductions:'0' });
  ...
  const Num = v => {
    ...
    const s = String(v).replace(/,(?=\d{1,2}($|\s))/,'.').replace(/[^\d.-]/g,'');
    const n = parseFloat(s);
    return isNaN(n) ? 0 : n;
  };
  const Rnd = v => Math.round((v || 0) * 100) / 100;
  const Fmt = v => '₦' + Rnd(v || 0).toLocaleString('en-NG', { minimumFractionDigits:2, maximumFractionDigits:2 });

  // ── PAYE 2026 BANDS ──
  const calcPAYE = T => {
    let tax = 0;
    tax += Math.max(0, Math.min(T, 3000000)  -  800000) * 0.15;
    tax += Math.max(0, Math.min(T, 12000000) - 3000000) * 0.18;
    tax += Math.max(0, Math.min(T, 25000000) -12000000) * 0.21;
    tax += Math.max(0, Math.min(T, 50000000) -25000000) * 0.23;
    tax += Math.max(0, T - 50000000)                    * 0.25;
    return tax;
  };

  // ── CORE CALCULATION ──
  // Step 1: Monthly Gross = Cumulative ÷ Duration
  //         10% Emp Pension = Monthly Gross × 70% × 10%
  //         Main Gross = Monthly Gross − 10% Pension
  // Step 2: Monthly NHIS = Main Gross × 1.5%
  //         Monthly 8% Pension = Main Gross × 70% × 8%
  // Step 3: Annual Gross = Main Gross × 12
  //         Annual Taxable = Annual Gross − (NHIS×12) − (8% Pension×12)
  //         Annual PAYE = calcPAYE(Annual Taxable)
  //         Monthly PAYE = Annual PAYE ÷ 12
  // Step 4: Net = Main Gross − Monthly PAYE − Monthly NHIS − Monthly 8% Pension
  const calcRow = s => {
    const dur  = Num(s.duration) || 6;
    const rate = Num(s.rate) || 1;
    const cur  = s.currency || 'NGN';
    const cumNGN = cur === 'NGN' ? Num(s.cumulative) : Num(s.cumulative) * rate;

    // Step 1
    // If foreign currency: round monthly amount to 1 decimal BEFORE converting to NGN
    // This matches the sample document rounding approach
    // Round monthly base to 1 decimal at division step — matches sample rounding
    const monthlyDiv   = N(s.cumulative) / dur;
    const monthlyRaw   = Math.round(monthlyDiv * 100) / 100;
    const monthlyBase  = cur === 'NGN' ? monthlyRaw : monthlyRaw * rate;
    const emp10        = Rnd(monthlyBase * 0.70 * 0.10);
    const mainGross    = Rnd(monthlyBase - emp10);

    // Step 2
    const nhisMo       = Rnd(mainGross * 0.015);
    const pens8Mo      = Rnd(mainGross * 0.70 * 0.08);

    // Step 3
    const annualGross  = mainGross * 12 + Num(s.otherIncome);
    const nhisAnn      = nhisMo * 12;
    const pens8Ann     = pens8Mo * 12;
    const rent         = Num(s.rent);
    const rentRelief   = rent > 0 ? Math.min(500000, Rnd(rent * 0.20)) : 0;
    const annTaxable   = Math.max(0, annualGross - nhisAnn - pens8Ann - rentRelief);
    const payeAnn      = Rnd(calcPAYE(annTaxable));
    const payeMo       = Rnd(payeAnn / 12);

    // Step 4
    const otherDed     = Num(s.otherDeductions);
    const netMo        = Rnd(mainGross - payeMo - nhisMo - pens8Mo - otherDed);

    return { name:s.name, position:s.position, cumNGN, dur, monthlyBase, emp10, mainGross,
             nhisMo, pens8Mo, annualGross, nhisAnn, pens8Ann, rentRelief, annTaxable,
             payeAnn, payeMo, otherDed, netMo };
  };
  ...
  const handleFile = async e => {
    ...
    reader.onload = ev => {
      ...
      // Find header row
      let hIdx = rows.findIndex(r => r.some(c => String(c).toLowerCase().match(/name|staff/)));
      if (hIdx === -1) hIdx = 0;
      const hdrs = rows[hIdx].map(c => String(c));
      const data = rows.slice(hIdx+1).filter(r => r.some(c => c !== ''));
      setHeaders(hdrs);
      setRawRows(data);
      setFileName(file.name);
      // Auto-detect columns
      const map = { name:-1, pos:-1, cum:-1, dur:-1, other:-1, rent:-1, ded:-1 };
      hdrs.forEach((h, i) => {
        const l = h.toLowerCase();
        if (l.match(/name|staff/) && map.name === -1)   map.name = i;
        if (l.match(/desig|pos|role|title/) && map.pos === -1) map.pos = i;
        if (l.match(/cum|gross|salary|figure|package/) && map.cum === -1) map.cum = i;
        if (l.match(/dur|month/) && map.dur === -1)     map.dur = i;
        if (l.match(/other.*inc/) && map.other === -1)  map.other = i;
        if (l.match(/rent/) && map.rent === -1)         map.rent = i;
        if (l.match(/deduct/) && map.ded === -1)        map.ded = i;
      });
      setColMap(map);
      setShowColMap(true);
    };
  ...
  const doUpload = () => {
    if (currency !== 'NGN' && !Num(exchangeRate)) { alert('Please enter the exchange rate.'); return; }
    const list = rawRows.map(row => {
      const name = String(row[colMap.name] || '').trim();
      if (!name) return null;
      return {
        name, position: String(row[colMap.pos] || ''),
        cumulative: row[colMap.cum],
        duration: colMap.dur >= 0 ? row[colMap.dur] : 6,
        otherIncome: colMap.other >= 0 ? row[colMap.other] : 0,
        rent: colMap.rent >= 0 ? row[colMap.rent] : 0,
        otherDeductions: colMap.ded >= 0 ? row[colMap.ded] : 0,
        currency, rate: Num(exchangeRate) || 1,
      };
    }).filter(Boolean);
    if (!list.length) { alert('No staff data found. Check column mapping.'); return; }
    setResults(list.map(calcRow));
    setTab('results');
  };

  // ── MANUAL ENTRY ──
  const addManual = () => {
    if (!form.name.trim() || !form.position.trim() || !form.cumulative.trim()) {
      alert('Please fill in Staff Name, Position and Cumulative Figure.'); return;
    }
    setManualStaff(prev => [...prev, { ...form, currency, rate: Num(exchangeRate) || 1 }]);
    setForm({ name:'', position:'', cumulative:'', duration:'6', otherIncome:'0', rent:'0', otherDeductions:'0' });
  };

  const calcManual = () => {
    if (!manualStaff.length) { alert('Add at least one staff member.'); return; }
    if (currency !== 'NGN' && !Num(exchangeRate)) { alert('Please enter the exchange rate.'); return; }
    const list = manualStaff.map(s => ({ ...s, currency, rate: Num(exchangeRate) || 1 }));
    setResults(list.map(calcRow));
    setTab('results');
  };
  ...
  const dlExcel = async () => {
    await loadXLSX();
    const hdrs = ['S/N','Staff Name','Position','Cumulative (NGN)','10% Emp Pension',
      'Main Monthly Gross','Duration','Monthly NHIS (1.5%)','Monthly Emp Pension (8%)',
      'Annual Gross','Rent Relief','Annual Emp Pension','Annual Taxable',
      'Annual PAYE','Monthly PAYE','Other Deductions','Net Pay (Monthly)'];
    const rows = results.map((r,i) => [
      i+1, r.name, r.position, Rnd(r.cumNGN), Rnd(r.emp10), Rnd(r.mainGross), r.dur,
      Rnd(r.nhisMo), Rnd(r.pens8Mo), Rnd(r.annualGross), Rnd(r.rentRelief),
      Rnd(r.pens8Ann), Rnd(r.annTaxable), Rnd(r.payeAnn), Rnd(r.payeMo),
      Rnd(r.otherDed), Rnd(r.netMo)
    ]);
    const ws = window.XLSX.utils.aoa_to_sheet([hdrs, ...rows]);
    ws['!cols'] = [4,24,22,16,14,16,8,14,16,14,12,14,14,12,12,12,14].map(w=>({wch:w}));
    const wb = window.XLSX.utils.book_new();
    window.XLSX.utils.book_append_sheet(wb, ws, 'Payroll');
    window.XLSX.writeFile(wb, `Payroll_${period}.xlsx`);
  };

  // ── TOTALS ──
  const totals = results.reduce((a, r) => ({
    cumNGN: a.cumNGN+r.cumNGN, emp10: a.emp10+r.emp10, mainGross: a.mainGross+r.mainGross,
    nhisMo: a.nhisMo+r.nhisMo, pens8Mo: a.pens8Mo+r.pens8Mo,
    payeAnn: a.payeAnn+r.payeAnn, payeMo: a.payeMo+r.payeMo, netMo: a.netMo+r.netMo
  }), { cumNGN:0,emp10:0,mainGross:0,nhisMo:0,pens8Mo:0,payeAnn:0,payeMo:0,netMo:0 });

  // ── STYLES ──
  const inputSt = { background:C.bgDeep, border:`1px solid ${C.border}`, borderRadius:8, padding:'9px 12px', color:C.text, fontFamily:'inherit', fontSize:13, outline:'none', width:'100%' };
  const selSt   = { ...inputSt };

  const months = ['January','February','March','April','May','June','July','August','September','October','November','December'];
  const nowD = new Date();
  const periodOptions = [];
  for (let y = nowD.getFullYear()-1; y <= nowD.getFullYear()+1; y++)
    for (let m = 0; m < 12; m++)
      periodOptions.push(`${y}-${String(m+1).padStart(2,'0')}`);

  return (
    <div>
      <h2 style={{ color:C.white, fontSize:24, fontWeight:800, marginBottom:8 }}>Payroll</h2>
      <p style={{ color:C.muted, fontSize:13, marginBottom:20 }}>Nigeria PAYE 2026 · Upload Excel or enter manually</p>

      {/* Tabs */}
      <div style={{ display:'flex', gap:6, marginBottom:18, background:C.bgDeep, border:`1px solid ${C.border}`, borderRadius:12, padding:5 }}>
        {[['upload','📤 Upload Excel'],['manual','✏️ Manual Entry'],['results','📊 Results']].map(([id,label]) => (
          id === 'results' && !results.length ? null :
          <button key={id} onClick={() => setTab(id)} style={{
            flex:1, padding:'10px 8px', borderRadius:8, border:'none', fontFamily:'inherit',
            fontSize:12, fontWeight:700, cursor:'pointer', transition:'all 0.2s',
            background: tab===id ? `linear-gradient(135deg,${C.success},#059669)` : 'transparent',
      ...
      {tab === 'upload' && (
        <div>
          {/* Currency */}
          <Card>
            <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:12 }}>Currency & Settings</h3>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:12 }}>
              <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                <label style={{ fontSize:11, color:C.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:0.5 }}>Currency</label>
                <select value={currency} onChange={e => setCurrency(e.target.value)} style={selSt}>
              ...
              {currency !== 'NGN' && (
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  <label style={{ fontSize:11, color:C.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:0.5 }}>{currency} → NGN Rate</label>
                  <input type="number" step="any" value={exchangeRate} onChange={e => setExchangeRate(e.target.value)} placeholder="e.g. 1580" style={inputSt} />
                </div>
              )}
              <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                <label style={{ fontSize:11, color:C.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:0.5 }}>Period</label>
          ...
          {/* Upload */}
          <Card>
            <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:12 }}>Upload Excel File</h3>
            <div style={{ background:C.accent+'15', border:`1px solid ${C.accent}33`, borderRadius:10, padding:'10px 14px', fontSize:12, color:C.accent, marginBottom:14, lineHeight:1.8 }}>
              📋 Required columns: <strong>Staff Name, Position, Cumulative Figure, Duration (months)</strong>
            </div>
            <div onClick={() => fileRef.current.click()} style={{
              border:`2px dashed ${fileName ? C.success : C.border}`, borderRadius:12, padding:'24px 20px',
              textAlign:'center', cursor:'pointer', background: fileName ? C.success+'06' : 'transparent', marginBottom:12
            }}>
              <div style={{ fontSize:32, marginBottom:8 }}>📄</div>
              <div style={{ color: fileName ? C.success : C.muted, fontSize:13, fontWeight:600 }}>
                {fileName ? `✓ ${fileName}` : 'Click or drag to upload Excel (.xlsx)'}
              </div>
            </div>
            <input ref={fileRef} type="file" accept=".xlsx,.xlsm,.xls" onChange={handleFile} style={{ display:'none' }} />

            {showColMap && headers.length > 0 && (
              <div>
                <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:12 }}>Map Columns</h3>
                <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:12, marginBottom:14 }}>
                  {[
                    ['name','Staff Name *'],['pos','Position *'],['cum','Cumulative Figure *'],
                    ['dur','Duration (months) *'],['other','Other Income'],['rent','Rent'],['ded','Other Deductions']
                  ].map(([key, label]) => (
                    <div key={key} style={{ display:'flex', flexDirection:'column', gap:5 }}>
                      <label style={{ fontSize:11, color:C.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:0.5 }}>{label}</label>
                      <select value={colMap[key]} onChange={e => setColMap(prev => ({ ...prev, [key]: parseInt(e.target.value) }))} style={selSt}>
                        <option value={-1}>-- Not in Excel --</option>
                        {headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
                      </select>
                    </div>
                  ))}
                </div>
                <Btn color={C.success} onClick={doUpload}>⚡ Calculate Payroll</Btn>
              </div>
            )}
          </Card>
        </div>
      ...
      {tab === 'manual' && (
        <div>
          ...
          <Card>
            <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:12 }}>Add Staff Member</h3>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:12, marginBottom:14 }}>
              {[
                ['name','Staff Name *','text','Full name'],
                ['position','Position *','text','e.g. Nurse'],
                ['cumulative','Cumulative Figure *','text','e.g. 4877178'],
                ['duration','Duration (months) *','number','6'],
                ['otherIncome','Other Income','text','0'],
                ['rent','Rent','text','0'],
                ['otherDeductions','Other Deductions','text','0'],
              ].map(([key, label, type, ph]) => (
                <div key={key} style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  <label style={{ fontSize:11, color:C.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:0.5 }}>{label}</label>
                  <input
                    type={type} inputMode={type === 'text' ? 'decimal' : undefined}
            ...
            <Btn color={C.success} onClick={addManual}>➕ Add Staff</Btn>
          </Card>

          {manualStaff.length > 0 && (
            <Card>
              <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:12 }}>Staff Added ({manualStaff.length})</h3>
              {manualStaff.map((s, i) => (
                <div key={i} style={{ background:C.bgDeep, border:`1px solid ${C.border}`, borderRadius:10, padding:'12px 14px', marginBottom:8, display:'flex', alignItems:'center', gap:12 }}>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700, fontSize:13, color:C.white }}>{s.name}</div>
                    <div style={{ fontSize:11, color:C.muted, marginTop:3 }}>
                      {s.position} · {s.currency} {Number(Num(s.cumulative)).toLocaleString()} · {s.duration} months
                    </div>
                  </div>
                  <Btn small color={C.danger} onClick={() => setManualStaff(prev => prev.filter((_,j) => j !== i))}>✕</Btn>
                </div>
              ))}
              <div style={{ display:'flex', gap:10, marginTop:12, flexWrap:'wrap' }}>
                <Btn color={C.success} onClick={calcManual}>⚡ Calculate Payroll</Btn>
                <Btn outline color={C.muted} onClick={() => setManualStaff([])}>🗑 Clear All</Btn>
              </div>
            </Card>
          )}
        </div>
      ...
      {tab === 'results' && results.length > 0 && (
        <div>
          {/* Summary */}
          <Card>
            <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase', marginBottom:12 }}>Summary</h3>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))', gap:10 }}>
              {[
                { label:'Total Staff',         val:results.length,           color:C.accent },
                { label:'Total 10% Pension',   val:Fmt(totals.emp10),        color:C.danger },
                { label:'Total Monthly Gross', val:Fmt(totals.mainGross),    color:C.accent },
                { label:'Total NHIS/Mo',        val:Fmt(totals.nhisMo),       color:C.warning },
                { label:'Total 8% Pension/Mo', val:Fmt(totals.pens8Mo),      color:C.danger },
                { label:'Total PAYE/Mo',        val:Fmt(totals.payeMo),       color:C.warning },
                { label:'Total Net Pay/Mo',     val:Fmt(totals.netMo),        color:C.success },
              ].map(({ label, val, color }) => (
                <div key={label} style={{ background:C.bgDeep, border:`1px solid ${C.border}`, borderRadius:10, padding:12, textAlign:'center' }}>
                  <div style={{ fontSize:13, fontWeight:800, color, marginBottom:3 }}>{val}</div>
                  <div style={{ fontSize:9, color:C.muted, fontWeight:600, textTransform:'uppercase', letterSpacing:0.5 }}>{label}</div>
                </div>
              ))}
            </div>
          </Card>
          ...
          <Card>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12, flexWrap:'wrap', gap:8 }}>
              <h3 style={{ color:C.success, fontSize:10, fontWeight:700, letterSpacing:1, textTransform:'uppercase' }}>Payroll Register</h3>
              <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                <Btn small color={C.success} onClick={dlExcel}>⬇️ Download Excel</Btn>
                <Btn small outline color={C.muted} onClick={() => { setResults([]); setTab('upload'); setFileName(''); setShowColMap(false); setManualStaff([]); }}>← New</Btn>
              </div>
            </div>
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11, minWidth:1400 }}>
                <thead>
                  <tr style={{ borderBottom:`2px solid ${C.border}` }}>
                    {['#','Staff Name','Position','Cumulative (NGN)','10% Emp Pension','Main Monthly Gross','Dur','Monthly NHIS','Monthly 8% Pension','Annual Gross','Rent Relief','Annual 8% Pension','Annual Taxable','Annual PAYE','Monthly PAYE','Other Ded','Net Pay (Monthly)'].map(h => (
                      <th key={h} style={{ padding:'9px 10px', textAlign: ['#','Staff Name','Position'].includes(h) ? 'left' : 'right', color:C.muted, fontSize:9, textTransform:'uppercase', fontWeight:700, whiteSpace:'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {results.map((r, i) => (
                    <tr key={i} style={{ borderBottom:`1px solid ${C.border}` }}>
                      ...
                      <td style={{ padding:'9px 10px', fontWeight:700, color:C.white, whiteSpace:'nowrap' }}>{r.name}</td>
                      <td style={{ padding:'9px 10px', color:C.muted, whiteSpace:'nowrap' }}>{r.position}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', whiteSpace:'nowrap' }}>{Fmt(r.cumNGN)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.danger, whiteSpace:'nowrap' }}>{Fmt(r.emp10)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.success, fontWeight:800, whiteSpace:'nowrap' }}>{Fmt(r.mainGross)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'center', whiteSpace:'nowrap' }}>{r.dur}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.warning, whiteSpace:'nowrap' }}>{Fmt(r.nhisMo)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.danger, whiteSpace:'nowrap' }}>{Fmt(r.pens8Mo)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', whiteSpace:'nowrap' }}>{Fmt(r.annualGross)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', whiteSpace:'nowrap' }}>{Fmt(r.rentRelief)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.danger, whiteSpace:'nowrap' }}>{Fmt(r.pens8Ann)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', whiteSpace:'nowrap' }}>{Fmt(r.annTaxable)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.warning, whiteSpace:'nowrap' }}>{Fmt(r.payeAnn)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.warning, fontWeight:700, whiteSpace:'nowrap' }}>{Fmt(r.payeMo)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', whiteSpace:'nowrap' }}>{Fmt(r.otherDed)}</td>
                      <td style={{ padding:'9px 10px', textAlign:'right', color:C.success, fontWeight:800, fontSize:12, whiteSpace:'nowrap' }}>{Fmt(r.netMo)}</td>
                    </tr>
                  ))}
                  {/* Totals row */}
                  <tr style={{ borderTop:`2px solid ${C.border}`, background:C.bgDeep }}>
                    <td colSpan={3} style={{ padding:'9px 10px', fontWeight:800, color:C.warning }}>TOTALS ({results.length} staff)</td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.warning }}>{Fmt(totals.cumNGN)}</td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.danger }}>{Fmt(totals.emp10)}</td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.success }}>{Fmt(totals.mainGross)}</td>
                    <td></td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.warning }}>{Fmt(totals.nhisMo)}</td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.danger }}>{Fmt(totals.pens8Mo)}</td>
                    <td colSpan={5}></td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.warning }}>{Fmt(totals.payeMo)}</td>
                    <td></td>
                    <td style={{ padding:'9px 10px', textAlign:'right', fontWeight:800, color:C.success, fontSize:13 }}>{Fmt(totals.netMo)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
...
function Payslips() {
  ...
  return (
    <div>
      ...
      {showSendModal && <PDFSendToStaff user={window.__hrUser} employees={window.__hrEmployees || []} docType="payslip" onClose={() => setShowSendModalP(false)} />}
    </div>
  );
}
...
function PDFSendToStaff({ user, employees, docType: initialDocType, onClose }) {
  ...
  // Fuzzy name match: returns best employee match from extracted PDF text
  const matchNameFromText = (text) => {
    const lines = text.split("\n").map(l => l.trim()).filter(l => l.length > 2);
    ...
    employees.forEach(emp => {
      ...
      lines.forEach(line => {
        ...
        // Score: how many parts of the employee name appear in this line
        const matches = empParts.filter(p => p.length > 2 && lineLow.includes(p));
        const score = matches.length / empParts.length;
  ...
  const buildStaff = async () => {
    ...
    // Load PDF.js and open the PDF for text extraction
    await loadPdfJs();
    let pdfDoc = null;
    try {
      ...
      console.warn("Could not open PDF for text extraction:", e);
    }
    ...
    for (let i = 0; i < cnt; i++) {
      ...
      // Extract text from the FIRST page of this staff group
      let matchedEmp = null;
      if (pdfDoc) {
        try {
          const page = await pdfDoc.getPage(fromPage);
          const textContent = await page.getTextContent();
          const pageText = textContent.items.map(item => item.str).join("\n");
          matchedEmp = matchNameFromText(pageText);
        } catch(e) {
          console.warn("Text extraction failed for page", fromPage, e);
        }
      }
      ...
      if (matchedEmp) {
        list.push({
          ...
          email: matchedEmp.official_email || "",
          pages: dp,
          fromPage,
          toPage,
          overridden: false,
          matchStatus: matchedEmp.official_email ? "exact" : "noemail",
          matchedEmp,
          overrideName: "",
        });
      } else {
          ...
          {step === 2 && (
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 18px", background: C.bgDeep, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 16, flexWrap: "wrap" }}>
                ...
                <Btn small color={C.success} onClick={async () => { await buildStaff(); setStep(3); setTimeout(renderSigPlacementPage.bind(null, sigPageNum), 200); }}>Apply & Next →</Btn>
              </div>
              {/* Thumbnails */}
      ...
      <div style={{ flex: 1, marginLeft: 220, padding: "32px 36px", overflowY: "auto", minHeight: "100vh" }} className="main-content">
        ...
        <div className="mobile-topbar" style={{ display: "none", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: C.bgDeep, borderBottom: `1px solid ${C.border}`, position: "sticky", top: 0, zIndex: 97, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button onClick={logout} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 8, padding: "5px 10px", color: C.muted, fontSize: 11, cursor: "pointer", fontWeight: 600 }}>
              Sign Out
            </button>
          </div>
