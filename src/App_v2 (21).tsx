...
function DocumentSigningModal({ notification, user, onClose, onSigned }) {
  ...
  const downloadPDF = async (sigUrl, staffName, docType, period) => {
    try {
      ...
      if (!doc || !doc.sig_x) {
        // Fallback to jsPDF if no original PDF data
        throw new Error("No original PDF data");
      }

      // We need the original PDF bytes — stored reference in doc
      // For now show toast - full PDF bytes integration in main flow
      alert("Document signed and returned to HR! HR can download the signed copy from their Notifications.");
    } catch(err) {
      // Fallback to simple confirmation
      console.error(err);
      alert("Document signed successfully and returned to HR!");
    }
  };
...
function PDFSendToStaff({ user, employees, docType: initialDocType, onClose }) {
  ...
  const buildStaff = () => {
    ...
    const list = [];
    for (let i = 0; i < cnt; i++) {
      const emp = employees[i] || { id: i + 1, name: `Staff ${i + 1}`, official_email: "" };
      list.push({
        id: i + 1,
        name: emp.name,
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16, overflowY: "auto" }}>
      <div style={{ background: C.card, borderRadius: 20, width: "min(860px,100%)", border: `1px solid ${C.border}`, boxShadow: "0 24px 80px rgba(0,0,0,0.7)", maxHeight: "94vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: 22 }}>
          ...
          {step === 4 && (
            <div>
              {mismatch && <div style={{ padding: "10px 14px", background: C.warning + "15", border: `1px solid ${C.warning}33`, borderRadius: 10, fontSize: 12, color: C.warning, marginBottom: 14 }}>{mismatch}</div>}
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ borderBottom: `2px solid ${C.border}` }}>
                      {["#", "Staff Name", "Email", "Pages", "Page Range", "Status"].map(h => (
                  ...
                  <tbody>
                    {staffList.map((s, i) => (
                      <tr key={i} style={{ borderBottom: `1px solid ${C.border}` }}>
                        ...
                        <td style={{ padding: "9px 12px" }}>
                          {s.email ? <Badge color={C.success}>✅ Ready</Badge> : <Badge color={C.warning}>📧 No Email</Badge>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
