...
function StaffDashboard({ user, employees, leaveRequests, isManager, setLeaveRequests }) {
  ...
  const [leaveForm, setLeaveForm] = useState({ type: "", from_date: "", to_date: "", reason: "", recipient_email: "", recipient_name: "", address_while_away: "" });
  ...
  const submitLeave = async () => {
    ...
    setLeaveForm({ type: "", from_date: "", to_date: "", reason: "", recipient_email: "", recipient_name: "", address_while_away: "" });
    setRecipientSearch("");
    setSubmitting(false);
  };
...
function Notifications({ user, employees, leaveRequests, setLeaveRequests }) {
  ...
  const filtered = filter === "all" ? notifs : notifs.filter(n => {
    if (filter === "documents") return n.notif_type === "contract" || n.notif_type === "payslip" || n.notif_type === "signed";
    if (filter === "leave") return n.notif_type?.startsWith("leave");
    ...
    <div>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(n => {
            const cfg = typeConfig[n.notif_type] || { icon: "🔔", color: C.accent, label: "" };
            return (
                ...
                <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
                      {(n.notif_type === "contract" || n.notif_type === "payslip") && (
                        <Btn small color={C.accent} onClick={e => { e.stopPropagation(); markRead(n.id); showToast("Opening document signing...", C.accent); }}>✍️ View & Sign</Btn>
                      )}
                      {n.notif_type === "signed" && (
                        <Btn small color={C.success} onClick={e => { e.stopPropagation(); markRead(n.id); }}>👁 View Signed Doc</Btn>
                      )}
                      {n.notif_type === "leave_request" && (
                        <>
                      ...
                      {(n.notif_type === "leave_approved" || n.notif_type === "leave_rejected" || n.notif_type === "leave_forwarded" || n.notif_type === "leave_cc") && (
