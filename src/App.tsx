...
function Contracts() {
  ...
  return (
    <div>
        ...
        <Card>
          ...
          {status.msg && status.type === "success" && (
            <Btn color={C.success} onClick={() => setShowSendModal(true)} style={{ width: "100%", padding: 12, fontSize: 13, justifyContent: "center", marginTop: 8 }}>
              📤 Send Contracts to Staff Dashboards
            </Btn>
          )}
          {status.msg && (
      ...
      {showSendModal && <SendToStaff user={window.__hrUser} employees={window.__hrEmployees || []} onClose={() => setShowSendModal(false)} />}
    </div>
  );
}
...
function Payslips() {
  ...
  return (
    <div>
        ...
        <Card>
          ...
          {status.msg && status.type === "success" && (
            <Btn color={C.accent} onClick={() => setShowSendModalP(true)} style={{ width:"100%", padding:12, fontSize:13, justifyContent:"center", marginTop:8 }}>
              📤 Send Payslips to Staff Dashboards
            </Btn>
          )}
          {status.msg && (
            <div style={{ marginTop:12, padding:"12px 16px", background:status.type==="success"?C.success+"15":status.type==="error"?C.danger+"15":C.accent+"15", border:`1px solid ${status.type==="success"?C.success:status.type==="error"?C.danger:C.accent}44`, borderRadius:10, color:status.type==="success"?C.success:status.type==="error"?C.danger:C.accent, fontSize:13 }}>
      ...
      {showSendModalP && <SendToStaff user={window.__hrUser} employees={window.__hrEmployees || []} onClose={() => setShowSendModalP(false)} />}
    </div>
  );
}
...
function DocumentSigningModal({ notification, user, onClose, onSigned }) {
  ...
  const downloadPDF = (sigUrl, staffName, docType, period) => {
    ...
    doc.text(docType === "contract" ? "EMPLOYMENT CONTRACT" : "MONTHLY PAYSLIP", W / 2, 16, { align: "center" });
    doc.setFontSize(8); doc.setFont("helvetica", "normal");
    doc.setTextColor(144, 202, 249);
    doc.text("CARE BEST INITIATIVE (CBI) — People Operations", W / 2, 23, { align: "center" });
    doc.text(period || "", W / 2, 30, { align: "center" });

    // Content
    let y = 48;
    const field = (label, value) => {
      doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(84, 110, 122);
      doc.text(label, m, y);
    ...
    doc.text("EMPLOYEE INFORMATION", m, y); y += 8;
    field("Staff Name:", staffName);
    field("Document Type:", docType === "contract" ? "Employment Contract" : "Monthly Payslip");
    field("Period:", period);
    field("Date Signed:", new Date().toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric" }));
    y += 6;

    // Signature section
    doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.setTextColor(26, 58, 107);
    doc.text("EMPLOYEE SIGNATURE", m, y); y += 8;
    doc.setDrawColor(176, 190, 197); doc.setLineWidth(0.5);
    doc.rect(m, y, cw / 2, 28);
    if (sigUrl) {
      try { doc.addImage(sigUrl, "PNG", m + 2, y + 2, cw / 2 - 4, 24); } catch (e) {}
    }
    y += 31;
    doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(84, 110, 122);
    doc.text(staffName || "", m, y);
    doc.text("Date: " + new Date().toLocaleDateString("en-GB"), m + cw / 2 + 5, y);

    // Signed stamp
    doc.setFontSize(14); doc.setFont("helvetica", "bold"); doc.setTextColor(16, 185, 129);
    doc.text("✓ SIGNED & RETURNED TO HR", m, y + 14);

    // Footer
    doc.setFillColor(13, 31, 60);
    doc.rect(0, H - 12, W, 12, "F");
    doc.setTextColor(144, 202, 249); doc.setFontSize(7);
    doc.text("CARE BEST INITIATIVE (CBI) — Signed Document — Confidential", W / 2, H - 4, { align: "center" });

    const filename = `Signed_${docType === "contract" ? "Contract" : "Payslip"}_${(staffName || "Staff").replace(/\s+/g, "_")}_${(period || "").replace(/\s+/g, "_")}.pdf`;
    doc.save(filename);
  };
        ...
        <div style={{ padding: 22, maxHeight: "80vh", overflowY: "auto" }}>
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ background: "white", borderRadius: 12, overflow: "hidden", marginBottom: 16, boxShadow: "0 4px 24px rgba(0,0,0,0.4)" }}>
                {/* Doc header */}
                <div style={{ background: "#0d1f3c", padding: "16px 20px", textAlign: "center" }}>
                  <div style={{ color: "white", fontWeight: 800, fontSize: 16 }}>
                    {isContract ? "EMPLOYMENT CONTRACT" : "MONTHLY PAYSLIP"}
                  </div>
                  <div style={{ color: "#90caf9", fontSize: 11, marginTop: 4 }}>
                    CARE BEST INITIATIVE (CBI) · People Operations · {docData?.period}
                  </div>
                  <div style={{ background: "rgba(16,185,129,0.2)", border: "1px solid rgba(16,185,129,0.4)", borderRadius: 6, padding: "4px 12px", display: "inline-block", marginTop: 8, color: "#10b981", fontSize: 11, fontWeight: 700 }}>
                    Sent by HR · {docData?.created_at ? new Date(docData.created_at).toLocaleDateString("en-GB") : ""}
                  </div>
                </div>

                {/* Employee details */}
                <div style={{ padding: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#1a3a6b", textTransform: "uppercase", letterSpacing: 0.5, borderBottom: "2px solid #1a3a6b", paddingBottom: 4, marginBottom: 12 }}>
                    Staff Information
                  </div>
                  {[
                    ["Staff Name", docData?.staff_name],
                    ["Document Type", isContract ? "Employment Contract" : "Monthly Payslip"],
                    ["Period", docData?.period],
                    ["Sent By", "HR Department — " + (docData?.sent_by || "")],
                    ["Date Issued", docData?.created_at ? new Date(docData.created_at).toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric" }) : ""],
                  ].map(([label, val]) => (
                    <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderBottom: "1px solid #f0f4f8" }}>
                      <span style={{ fontSize: 12, color: "#546e7a", fontWeight: 600 }}>{label}</span>
                      <span style={{ fontSize: 12, color: "#1a1a2e", fontWeight: 700, textAlign: "right", maxWidth: "60%" }}>{val || "—"}</span>
                    </div>
                  ))}
                  ...
                  {isContract && (
                    <div style={{ marginTop: 16 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#546e7a", lineHeight: 1.8, background: "#f8fafc", padding: 12, borderRadius: 8 }}>
                        This agreement is for <strong>Six (6) months</strong>. The Employee agrees to be employed in accordance with CBI salary grid and is subject to all statutory deductions including PAYE, NHIS and Pension. By signing, the Employee confirms they have read and understood all contract terms.
                      </div>
                    </div>
                  )}

                  {/* Payslip specific content */}
                  {!isContract && (
                    <div style={{ marginTop: 16 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#1a3a6b", textTransform: "uppercase", letterSpacing: 0.5, borderBottom: "2px solid #1a3a6b", paddingBottom: 4, marginBottom: 12 }}>
                        Payment Summary
                      </div>
                      <div style={{ fontSize: 12, color: "#546e7a", background: "#f8fafc", padding: 12, borderRadius: 8, lineHeight: 2 }}>
                        Please review your payslip carefully. If you notice any discrepancies in your salary, deductions or allowances, please contact HR immediately before signing.
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ marginTop: 16 }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#1a3a6b", textTransform: "uppercase", letterSpacing: 0.5, borderBottom: "2px solid #1a3a6b", paddingBottom: 4, marginBottom: 10 }}>
                      Employee Signature
                    </div>
                    <div style={{ border: "2px dashed #b0bec5", borderRadius: 8, height: 70, display: "flex", alignItems: "center", justifyContent: "center", background: "#f8fafc" }}>
                      <span style={{ color: "#90a4ae", fontSize: 12 }}>Your signature will appear here after signing</span>
                    </div>
                  </div>
                </div>
              </div>

              <div style={{ padding: "10px 14px", background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.3)", borderRadius: 10, fontSize: 12, color: "#f59e0b", marginBottom: 16, lineHeight: 1.7 }}>
          ...
          <div style={{ background:C.card, borderRadius:20, width:"min(600px,100%)", border:`1px solid ${C.border}`, boxShadow:"0 24px 80px rgba(0,0,0,0.7)", maxHeight:"90vh", overflowY:"auto" }}>
            ...
            <div style={{ padding:22 }}>
              ...
              <div style={{ display:"flex", gap:10, justifyContent:"center" }}>
                <Btn color={C.success} onClick={() => {
                  if (!window.jspdf) { alert("PDF library not loaded yet, please try again"); return; }
                  const { jsPDF } = window.jspdf;
                  const doc = new jsPDF({ orientation:"portrait", unit:"mm", format:"a4" });
                  const W=210, H=297, m=20, cw=W-2*m;
                  doc.setFillColor(13,31,60); doc.rect(0,0,W,38,"F");
                  doc.setFillColor(200,169,110); doc.rect(0,38,W,1.5,"F");
                  doc.setTextColor(255,255,255); doc.setFontSize(16); doc.setFont("helvetica","bold");
                  doc.text(signedDocData.document_type==="contract"?"EMPLOYMENT CONTRACT":"MONTHLY PAYSLIP", W/2, 16, {align:"center"});
                  doc.setFontSize(8); doc.setFont("helvetica","normal"); doc.setTextColor(144,202,249);
                  doc.text("CARE BEST INITIATIVE (CBI) — People Operations", W/2, 23, {align:"center"});
                  doc.text(signedDocData.period||"", W/2, 30, {align:"center"});
                  let y=48;
                  doc.setFontSize(10); doc.setFont("helvetica","bold"); doc.setTextColor(26,58,107);
                  doc.text("EMPLOYEE INFORMATION", m, y); y+=8;
                  [[`Staff Name:`,signedDocData.staff_name],[`Period:`,signedDocData.period],[`Date Signed:`,signedDocData.signed_at?new Date(signedDocData.signed_at).toLocaleDateString("en-GB"):""]].forEach(([l,v])=>{ doc.setFontSize(8); doc.setFont("helvetica","normal"); doc.setTextColor(84,110,122); doc.text(l,m,y); doc.setFont("helvetica","bold"); doc.setTextColor(26,26,46); doc.text(String(v||""),m+55,y); y+=8; });
                  y+=6; doc.setFontSize(10); doc.setFont("helvetica","bold"); doc.setTextColor(26,58,107);
                  doc.text("EMPLOYEE SIGNATURE", m, y); y+=8;
                  doc.setDrawColor(176,190,197); doc.setLineWidth(0.5); doc.rect(m,y,cw/2,28);
                  if (signedDocData.signature_data) { try { doc.addImage(signedDocData.signature_data,"PNG",m+2,y+2,cw/2-4,24); } catch(e){} }
                  y+=31; doc.setFontSize(8); doc.setFont("helvetica","normal"); doc.setTextColor(84,110,122);
                  doc.text(signedDocData.staff_name||"",m,y);
                  doc.text("Date: "+(signedDocData.signed_at?new Date(signedDocData.signed_at).toLocaleDateString("en-GB"):""),m+cw/2+5,y);
                  y+=10; doc.setFontSize(12); doc.setFont("helvetica","bold"); doc.setTextColor(16,185,129);
                  doc.text("SIGNED AND RETURNED TO HR",m,y);
                  doc.setFillColor(13,31,60); doc.rect(0,H-12,W,12,"F");
                  doc.setTextColor(144,202,249); doc.setFontSize(7);
                  doc.text("CARE BEST INITIATIVE (CBI) — Signed Document — Confidential",W/2,H-4,{align:"center"});
                  doc.save("Signed_"+signedDocData.staff_name.replace(/ /g,"_")+".pdf");
...
// ── SEND TO STAFF ─────────────────────────────────────────────────────────────
function SendToStaff({ user, employees, onClose }) {
  ...
  const [progress, setProgress] = useState(0);
  const [results, setResults] = useState({ sent: 0, skipped: 0 });
  ...
  const buildMatchList = () => {
    // Use employee names from Supabase as the source
    const list = employees.map((emp, i) => {
      const email = emp.official_email || "";
      return {
        id: i + 1,
        excelName: emp.name,
        matchedName: emp.name,
        email,
        status: email ? "exact" : "noemail",
        empId: emp.id,
      };
    });
    setStaffList(list);
  ...
  const fixEmail = (id, email) => {
    setStaffList(staffList.map(s => s.id === id ? { ...s, email, status: email ? "exact" : "noemail" } : s));
  };
  ...
  const sendAll = async () => {
    ...
    for (let i = 0; i < toSend.length; i++) {
      ...
      sent++;
      setProgress(Math.round((sent / toSend.length) * 100));
    }

    const skipped = staffList.filter(s => !s.email).length;
  ...
  const toSend = staffList.filter(s => s.email);
  const toSkip = staffList.filter(s => !s.email);
  ...
  for (let y = now.getFullYear() - 1; y <= now.getFullYear() + 1; y++)
    for (let m = 0; m < 12; m++)
      periodOptions.push({ value: `${months[m]} ${y}`, label: `${months[m]} ${y}` });

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: C.card, borderRadius: 20, width: "min(900px,100%)", maxHeight: "90vh", overflowY: "auto", border: `1px solid ${C.border}`, boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
        {/* Header */}
        <div style={{ padding: "20px 24px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: C.card, zIndex: 10 }}>
          <div>
            <h2 style={{ color: C.white, fontSize: 18, fontWeight: 800, margin: 0 }}>📤 Send Documents to Staff</h2>
            <p style={{ color: C.muted, fontSize: 12, marginTop: 3 }}>Send contracts or payslips directly to each staff member's dashboard</p>
          </div>
          <Btn small color={C.border} onClick={onClose}>✕ Close</Btn>
        </div>

        <div style={{ padding: 24 }}>
          {/* Step 1 - Select type and period */}
          {step === 1 && (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12, marginBottom: 20 }}>
                {[["contract","📄","Employment Contracts","Send individual contracts to all staff"],["payslip","💰","Monthly Payslips","Send payslips to all staff"]].map(([type,icon,title,sub]) => (
                  <div key={type} onClick={() => setDocType(type)} style={{ padding: 18, background: C.bgDeep, border: `2px solid ${docType === type ? C.success : C.border}`, borderRadius: 14, cursor: "pointer", textAlign: "center", transition: "all 0.2s" }}>
              ...
              <Select label="Document Period" value={period} onChange={setPeriod} options={periodOptions} />
              <div style={{ marginTop: 16, textAlign: "right" }}>
                <Btn color={C.success} onClick={buildMatchList}>Next: Review Staff →</Btn>
              </div>
            </div>
          )}

          {/* Step 2 - Match and review */}
          {step === 2 && (
            <div>
              {/* Stats */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10, marginBottom: 16 }}>
                {[
                  { label: "Total Staff", value: staffList.length, color: C.accent },
                  { label: "✅ Ready to Send", value: toSend.length, color: C.success },
                  { label: "📧 No Email", value: toSkip.length, color: C.warning },
                ].map(({ label, value, color }) => (
                  <div key={label} style={{ background: color+"15", border: `1px solid ${color}33`, borderRadius: 10, padding: "12px 14px", textAlign: "center" }}>
                    <div style={{ fontSize: 22, fontWeight: 800, color, fontFamily: "monospace" }}>{value}</div>
              ...
              {toSkip.length > 0 && (
                <div style={{ padding: "10px 14px", background: C.warning+"15", border: `1px solid ${C.warning}33`, borderRadius: 10, fontSize: 12, color: C.warning, marginBottom: 14 }}>
                  ⚠️ {toSkip.length} staff have no email on record and will be skipped. Update their email in the Employees module.
                </div>
              )}
                  ...
                  <tbody>
                    {staffList.map(s => (
                      <tr key={s.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                        <td style={{ padding: "10px 12px", color: C.muted, fontSize: 11 }}>{s.id}</td>
                        <td style={{ padding: "10px 12px", fontWeight: 600 }}>{s.matchedName}</td>
                        <td style={{ padding: "10px 12px", color: s.email ? C.muted : C.danger, fontSize: 12 }}>{s.email || "Not found"}</td>
                        <td style={{ padding: "10px 12px" }}>
                          {s.email
                            ? <Badge color={C.success}>✅ Ready</Badge>
                            : <Badge color={C.warning}>📧 No Email</Badge>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginTop: 20, flexWrap: "wrap", gap: 10 }}>
                <Btn outline color={C.muted} onClick={() => setStep(1)}>← Back</Btn>
                <Btn color={C.success} onClick={sendAll} disabled={!toSend.length || sending}>
                  {sending ? `Sending... ${progress}%` : `📤 Send to ${toSend.length} Staff`}
                </Btn>
              </div>
              ...
              {sending && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 4, textAlign: "center" }}>Sending documents and creating notifications...</div>
                </div>
              )}
            </div>
          )}
          ...
          {step === 3 && (
            <div style={{ textAlign: "center", padding: "32px 16px" }}>
              ...
              <div style={{ color: C.white, fontFamily: "inherit", fontSize: 22, fontWeight: 800, marginBottom: 8 }}>Documents Sent!</div>
              <div style={{ color: C.muted, fontSize: 13, marginBottom: 24, lineHeight: 1.8 }}>
                All matched staff have received notifications in their dashboard.<br />
                They will be prompted to review, sign and return when they log in.
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
                <div style={{ background: C.success+"15", border: `1px solid ${C.success}33`, borderRadius: 12, padding: 16 }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: C.success, fontFamily: "monospace" }}>{results.sent}</div>
                  <div style={{ fontSize: 12, color: C.success, fontWeight: 600, marginTop: 4 }}>Documents Sent</div>
                </div>
                <div style={{ background: C.warning+"15", border: `1px solid ${C.warning}33`, borderRadius: 12, padding: 16 }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: C.warning, fontFamily: "monospace" }}>{results.skipped}</div>
                  <div style={{ fontSize: 12, color: C.warning, fontWeight: 600, marginTop: 4 }}>Skipped (No Email)</div>
                </div>
              </div>
