...
function StaffDashboard({ user, employees, leaveRequests, isManager, setLeaveRequests }) {
  ...
  const [leaveForm, setLeaveForm] = useState({ type: "", from_date: "", to_date: "", reason: "", recipient_email: "", recipient_name: "" });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (me) {
        // Load attendance
        const { data: att } = await supabase.from("attendance").select("*").eq("employee_id", me.id).order("date", { ascending: false });
        setAttendanceRecords(att || []);

        // Load leave balance
        const { data: bal } = await supabase.from("leave_balances").select("*").eq("employee_id", me.id).single().catch(() => ({ data: null }));
        setLeaveBalance(bal);

        // Load profile picture from storage
        const { data: picData } = await supabase.storage.from("profile-pictures").getPublicUrl(`${me.id}.jpg`);
        if (picData?.publicUrl) setProfilePic(picData.publicUrl + "?t=" + Date.now());
      }

      // If manager load requests sent to them
      if (isManager) {
        const { data: mgrReqs } = await supabase.from("leave_requests").select("*").eq("recipient_email", user.email).eq("status", "Pending - Line Manager");
  ...
  const uploadPicture = async (file) => {
    ...
    setUploading(true);
    try {
      await supabase.storage.from("profile-pictures").upload(`${staffRecord.id}.jpg`, file, { upsert: true, contentType: file.type });
      const { data } = await supabase.storage.from("profile-pictures").getPublicUrl(`${staffRecord.id}.jpg`);
  ...
  const submitLeave = async () => {
    ...
    if (data?.[0]) setLeaveRequests(prev => [...prev, data[0]]);
    setShowConfirm(false);
    setShowLeaveForm(false);
    setLeaveForm({ type: "", from_date: "", to_date: "", reason: "", recipient_email: "", recipient_name: "" });
    setRecipientSearch("");
    setSubmitting(false);
  };
    ...
    <div>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>
          {isManager ? "Line Manager Dashboard" : "My Dashboard"} — {new Date().toLocaleDateString("en", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
        </p>
      </div>
...
export default function App() {
  ...
  const visibleModules = isHR ? allModules : allModules.filter(m => ["dashboard", "leave"].includes(m.id));
