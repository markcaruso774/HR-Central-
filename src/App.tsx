...
function StaffDashboard({ user, employees, leaveRequests, isManager, setLeaveRequests }) {
    ...
    const load = async () => {
      try {
        ...
        if (me) {
          ...
          // Load profile picture — try .png first then .jpg
          try {
            const filePath = `${me.id}.png`;
            const { data: picData } = supabase.storage.from("profile-pictures").getPublicUrl(filePath);
            if (picData?.publicUrl) {
              // Verify it actually exists by checking the URL
              const res = await fetch(picData.publicUrl, { method: "HEAD" }).catch(() => null);
              if (res?.ok) setProfilePic(picData.publicUrl + "?t=" + Date.now());
              else {
                // Try .jpg fallback
                const { data: jpgData } = supabase.storage.from("profile-pictures").getPublicUrl(`${me.id}.jpg`);
                const res2 = await fetch(jpgData.publicUrl, { method: "HEAD" }).catch(() => null);
                if (res2?.ok) setProfilePic(jpgData.publicUrl + "?t=" + Date.now());
              }
            }
          } catch {}
        }
  ...
  const uploadPicture = async (file) => {
    ...
    setUploading(true);
    try {
      // Always save as .png for consistency regardless of input format
