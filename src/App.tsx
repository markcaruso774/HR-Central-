...
function DocumentSigningModal({ notification, user, onClose, onSigned }) {
  const [step, setStep] = useState(1); // 1=review, 2=sign, 3=done
  ...
  useEffect(() => {
    if (step === 2) setTimeout(() => setupCanvas(), 100);
  }, [step]);
  ...
  const undoSig = () => {
    ...
    setSigDataUrl(newPaths.length > 0 ? canvas.toDataURL() : null);
  };
  ...
  const submitSignature = async () => {
    ...
    setSubmitting(true);
    try {
      // Save signature to documents table
      if (notification?.document_id) {
      ...
      // Notify HR that document was signed
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 1100, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: C.card, borderRadius: 20, width: "min(700px,100%)", maxHeight: "92vh", overflowY: "auto", border: `1px solid ${C.border}`, boxShadow: "0 24px 80px rgba(0,0,0,0.7)" }}>
        ...
        <div style={{ padding: "18px 22px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: C.card, zIndex: 10 }}>
          <div>
            <h2 style={{ color: C.white, fontSize: 17, fontWeight: 800, margin: 0 }}>
              {step === 1 ? "📄 Review Document" : step === 2 ? "✍️ Sign Document" : "✅ Document Signed"}
            </h2>
            <p style={{ color: C.muted, fontSize: 12, marginTop: 3 }}>
              {docData?.document_type === "contract" ? "Employment Contract" : "Monthly Payslip"} · {docData?.period || ""}
            </p>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {/* Step indicators */}
            {[1,2,3].map(s => (
              <div key={s} style={{ width: 28, height: 28, borderRadius: "50%", background: step >= s ? C.success : C.border, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700 }}>
                {step > s ? "✓" : s}
              </div>
            ))}
        ...
        <div style={{ padding: 22 }}>
          ...
          {step === 1 && (
            <div>
              <div style={{ background: "white", borderRadius: 12, padding: 22, marginBottom: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
                {/* Doc header */}
                <div style={{ background: "#0d1f3c", borderRadius: "8px 8px 0 0", padding: "14px 18px", margin: "-22px -22px 16px -22px" }}>
                  <div style={{ color: "white", fontWeight: 800, fontSize: 15, textAlign: "center" }}>
                    {docData?.document_type === "contract" ? "EMPLOYMENT CONTRACT" : "MONTHLY PAYSLIP"}
                  </div>
                  <div style={{ color: "#90caf9", fontSize: 11, textAlign: "center", marginTop: 3 }}>
                    CARE BEST INITIATIVE (CBI) · {docData?.period}
                  </div>
                </div>
                ...
                {[
                  ...
                  ["Document Type", docData?.document_type === "contract" ? "Employment Contract" : "Monthly Payslip"],
                  ["Period", docData?.period],
                  ["Sent By", "HR Department"],
                  ["Date Sent", docData?.created_at ? new Date(docData.created_at).toLocaleDateString("en-GB", { weekday: "long", year: "numeric", month: "long", day: "numeric" }) : ""],
                ].map(([label, val]) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #f0f4f8" }}>
                    ...
                    <span style={{ fontSize: 12, color: "#1a1a2e", fontWeight: 700 }}>{val || "—"}</span>
                  </div>
                ))}

                <div style={{ marginTop: 14, padding: "10px 14px", background: "#fff8e1", borderRadius: 8, fontSize: 12, color: "#f57f17", lineHeight: 1.7 }}>
                  ⚠️ By signing this document you confirm you have read and understood all its contents. Your signature will be embedded into the document and returned to HR.
                </div>
              </div>
          ...
          {step === 2 && (
            <div>
              <p style={{ color: C.muted, fontSize: 13, marginBottom: 16, lineHeight: 1.7 }}>
                Draw your signature below using your finger or mouse. This will be permanently embedded into the document.
              </p>
              ...
              <div style={{ display: "flex", gap: 20, marginBottom: 14, flexWrap: "wrap" }}>
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {penColors.map(({ color, label }) => (
                      <div key={color} onClick={() => setPenColor(color)} title={label}
                        style={{ width: 28, height: 28, borderRadius: "50%", background: color, cursor: "pointer", border: penColor === color ? "3px solid white" : "3px solid transparent", transform: penColor === color ? "scale(1.2)" : "scale(1)", transition: "all 0.15s" }} />
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    {[{ size: 1.5, w: 8 }, { size: 2.5, w: 12 }, { size: 4, w: 16 }].map(({ size, w }) => (
                      <div key={size} onClick={() => setPenSize(size)}
                        style={{ width: w, height: w, borderRadius: "50%", background: C.text, cursor: "pointer", border: penSize === size ? `2px solid ${C.accent}` : "2px solid transparent", transition: "all 0.15s" }} />
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                ...
                <Btn small outline color={C.muted} onClick={undoSig}>↩ Undo</Btn>
              </div>

              {/* Typed signature option */}
              <div style={{ padding: 14, background: C.bgDeep, borderRadius: 10, border: `1px solid ${C.border}`, marginBottom: 16 }}>
              ...
              {/* Preview */}
              {sigDataUrl && (
                <div style={{ padding: 14, background: C.success+"10", border: `1px solid ${C.success}33`, borderRadius: 10, marginBottom: 16 }}>
                  <div style={{ fontSize: 11, color: C.success, fontWeight: 700, marginBottom: 8 }}>✓ Signature Preview</div>
                  <img src={sigDataUrl} alt="Signature preview" style={{ maxHeight: 60, maxWidth: "100%", background: "white", borderRadius: 6, padding: 6 }} />
                </div>
              )}
          ...
          {step === 3 && (
            <div style={{ textAlign: "center", padding: "32px 16px" }}>
              ...
              <div style={{ color: C.white, fontSize: 20, fontWeight: 800, marginBottom: 8 }}>Document Signed Successfully!</div>
              <div style={{ color: C.muted, fontSize: 13, lineHeight: 1.8, marginBottom: 24 }}>
                Your signature has been embedded into the document<br />and returned to HR. You will receive a confirmation shortly.
              </div>
              <div style={{ background: C.bgDeep, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 20, fontSize: 13, lineHeight: 2, textAlign: "left" }}>
                📄 <strong style={{ color: C.text }}>Document:</strong> <span style={{ color: C.muted }}>{docData?.document_type === "contract" ? "Employment Contract" : "Payslip"} · {docData?.period}</span><br />
              ...
              <div style={{ padding: "10px 14px", background: C.accent+"15", border: `1px solid ${C.accent}33`, borderRadius: 10, fontSize: 12, color: C.accent, marginBottom: 20 }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(n => {
            ...
            return (
              <div key={n.id} onClick={() => markRead(n.id)} style={{ background: n.is_read ? C.card : `linear-gradient(135deg, ${cfg.color}08, ${C.card})`, border: `1px solid ${n.is_read ? C.border : cfg.color+"44"}`, borderLeft: `3px solid ${n.is_read ? C.border : cfg.color}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", transition: "all 0.2s", position: "relative" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
                      {(n.type === "contract" || n.type === "payslip") && (
                        <Btn small color={C.accent} onClick={e => { e.stopPropagation(); markRead(n.id); setSigningNotif(n); }}>✍️ View & Sign</Btn>
                      )}
                      {n.type === "signed" && (
                        <Btn small color={C.success} onClick={e => { e.stopPropagation(); markRead(n.id); }}>👁 View Signed Doc</Btn>
                      )}
                      {n.type === "leave_request" && (
                        <>
                          <Btn small color={C.success} onClick={e => { e.stopPropagation(); approveLeave(n); }}>✅ Approve</Btn>
                          <Btn small color={C.danger} onClick={e => { e.stopPropagation(); rejectLeave(n); }}>❌ Reject</Btn>
                          <Btn small outline color={C.muted} onClick={e => { e.stopPropagation(); markRead(n.id); }}>👁 View</Btn>
                        </>
                      )}
                      {(n.type === "leave_approved" || n.type === "leave_rejected" || n.type === "leave_forwarded" || n.type === "leave_cc") && (
                        <Btn small outline color={C.muted} onClick={e => { e.stopPropagation(); markRead(n.id); }}>👁 View Details</Btn>
                      )}
                      <Btn small color={C.danger} onClick={e => { e.stopPropagation(); dismiss(n.id); }}>Dismiss</Btn>
                    </div>
                  </div>
                </div>
              </div>
