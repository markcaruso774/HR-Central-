...
function Payroll({ employees }) {
  ...
  const handleFile = async e => {
    ...
    reader.onload = ev => {
      ...
      hdrs.forEach((h, i) => {
        ...
        if (l.match(/name|staff|employee|personnel/) && map.name === -1)    map.name = i;
        if (l.match(/desig|pos|role|title|dept|grade/) && map.pos === -1)   map.pos = i;
        if (l.match(/cum|gross|salary|figure|package|amount|total/) && map.cum === -1) map.cum = i;
        if (l.match(/dur|month|period|length/) && map.dur === -1)           map.dur = i;
        if (l.match(/other|allow|bonus/) && map.other === -1)               map.other = i;
        if (l.match(/rent|housing/) && map.rent === -1)                     map.rent = i;
        if (l.match(/deduct|less/) && map.ded === -1)                       map.ded = i;
      });
      // Fallback: if required columns still not found, default to first two columns
      if (map.name === -1) map.name = 0;
      if (map.cum === -1 && hdrs.length > 1) map.cum = 1;
      setColMap(map);
      setShowColMap(true);
    };
  ...
  const doUpload = () => {
    ...
    if (colMap.name < 0) { alert('Staff Name column not mapped. Please select it manually.'); return; }
    if (colMap.cum < 0) { alert('Cumulative Figure column not mapped. Please select it manually.'); return; }
