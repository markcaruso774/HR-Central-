...
// ── REAL PDF VIEWER ──────────────────────────────────────────────────────────
function RealPDFViewer({ docData }) {
  ...
  const renderPages = async () => {
    try {
      ...
      // Use global PDF doc or bytes
      let pdfDoc = window.__hrPdfDoc;
      if (!pdfDoc && window.__hrPdfBytes) {
        pdfDoc = await window.pdfjsLib.getDocument(new Uint8Array(window.__hrPdfBytes.slice(0))).promise;
        window.__hrPdfDoc = pdfDoc;
      }

      if (!pdfDoc) {
        setError("PDF not available. Please ensure HR has sent the document correctly.");
        setLoading(false);
        return;
      }
      ...
      // Get this staff's page range from docData
      const fromPage = docData?.from_page || 1;
      const toPage = docData?.to_page || pdfDoc.numPages;
